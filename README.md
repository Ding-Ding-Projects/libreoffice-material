...
| Canonical rewrite coverage | Contracted; implementation in progress | The operator-provided design ZIP is pinned by hash in the [`Windows rewrite contract`](docs/design/00-windows-rewrite-contract.md). Its eleven surfaces govern the native Windows rewrite. Fail-closed registries currently track all 599 top-level LibreOffice dialog roots (with explicit modal exclusions for input/destructive/credential/security prompts), 30 audited shipping text-query fields, a 1271-surface UI-closure ledger, dialog anatomy, eight routed notification producers, Impress/Draw surfaces, and the wave-2 shell/navigation contracts. Fifteen of the 30 search fields are source-integrated with the shared adjacent regex builder and 15 carry documented architectural gaps. The 2026-07-31 slices normalize the search dialog primary response, improve the field-description panel, correct its Base child-window classification, and add Material inset grids plus ordered, bounded, or exact-occurrence C++ page-host proof for twenty-one runtime-composed dialog shells, including the runtime-titled Chart Object Attributes shell, modeless Hyperlink and Writer Fields shells, and specialized PDF, Document Properties, and Writer-format dependencies. A fail-closed ownership contract also pins the five pending native shells and shared wizard shell while requiring them to remain uncredited until implementation evidence exists. The commit-auditable source headline is now 84.26% (1071/1271), with 200 pending. These are source/static/build-free facts; native and runtime proof remain separate gates. |
