...
| Windows updater | Material lifecycle source plus protected staging; end-user flow not yet exercised | Eleven stable states are mapped, while eligible availability/progress/pause/retry/completion summaries route through the shared bottom-right Material notification stack after worker→main-thread marshalling. Routine checking/no-update paths remain quiet under the existing suppression policy; the update icon remains the action path, error cards persist, same-state progress does not spam, and Windows Installer is named as completion/rollback owner. With no bundled verified code-name catalog/feed field, version-only fallback invents nothing. The updater still reads the exact GitHub Latest XML asset, rejects untrusted/legacy state, verifies canonical MSI metadata and bytes, stages through protected LocalAppData, and requires default-No consent before a visible install. The major-update vector remains exactly `/i`, staged MSI, `REBOOT=ReallySuppress`, and `MSIRESTARTMANAGERCONTROL=DisableShutdown`; repair-only `REINSTALL` properties are excluded. Source contracts pass; notification/download/consent/visible-launch and real MSI lifecycle proof remain pending. See the [feature article](docs/features/updater-lifecycle.md) and [Privacy](PRIVACY.md). |
| Installer / release | MSI-132 is Latest; further complete-type correction rerun pending | The one-time repair restored `windows-msi-123-1-952090ce26` at `952090ce2` as Latest with exactly four assets, and canonical unauthenticated MSI/XML/checksum routes returned HTTP 200 with 197,111,808/960/103-byte lengths. The serialized ancestry-guarded publisher has since advanced Latest to `windows-msi-132-1-90e5ea4f1e` at `90e5ea4f1e`, also with four assets. Source releases are non-Latest; the Windows UI contract excludes release-tag pushes; and CI enforces fleet/release-channel closure. Three legacy unguarded MSI runs were cancelled and Latest was rechecked. Source run `30214506688` verified the exact-tag preflight; initial MSI run `30214506398` exposed GitHub's 21,000-character expression limit before job creation, and current source removes inline expressions from that 23 KB publish body. The source workflow has published at least 21 packages, but the packaged build-from-source script remains unverified end to end. The disposable lifecycle harness now acquires pinned assets only through `gh release download` and launches Windows Sandbox on a run-scoped Lowlevel MCP off-screen desktop; its static validators pass, while a fresh lifecycle result remains open. MSI-123 remains historical build evidence for Windows-only stages 4+5, restored document-tab Stage 3, and UI scale; run `30427865981` compile-confirmed the earlier `MapUnit` fix, while the modified document-tab owner now awaits an exact-SHA rerun of its complete-type correction. Tabs remain runtime-unverified and scale remains stored-only. See [release-channel integrity](docs/build/release-channel-integrity.md). |
