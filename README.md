...
| Canonical rewrite coverage | Contracted; implementation in progress | The operator-provided design ZIP is pinned by hash in the [`Windows rewrite contract`](docs/design/00-windows-rewrite-contract.md). Its eleven surfaces govern the native Windows rewrite. Fail-closed registries currently track all 599 top-level LibreOffice dialog roots (with explicit modal exclusions for input/destructive/credential/security prompts), 30 audited shipping text-query fields, a 1271-surface UI-closure ledger, dialog anatomy, eight routed notification producers, Impress/Draw surfaces, and the wave-2 shell/navigation contracts. Fifteen of the 30 search fields are source-integrated with the shared adjacent regex builder and 15 carry documented architectural gaps. The 2026-07-31 slices add Material inset grids plus ordered, bounded, or exact-occurrence C++ page-host proof for twenty-one runtime-composed dialog shells. A separate hash-locked composition contract corrects the one-size-fits-all static-family defect for 194 audited runtime/modeless/lifecycle dialogs and host/atomic fragments: 184 retain prior adversarial `blocked-confirmed` evidence and 10 were re-audited from current source, with every resource tied to the globally activated Material renderer without fake labels, changed modality, or broken host geometry. The shared runtime wizard now consumes token-derived page spacing and marks exactly Next/Finish as Material forward actions. Every `.ui`-backed family is complete. A fail-closed ownership contract retains only the five pending native shells. The commit-auditable source headline is now 99.61% (1266/1271), with 5 pending. These are source/static/build-free facts; native and runtime proof remain separate gates. |
