...
| Canonical rewrite coverage | Contracted; implementation in progress | The operator-provided design ZIP is pinned by hash in the [`Windows rewrite contract`](docs/design/00-windows-rewrite-contract.md). Its eleven surfaces govern the native Windows rewrite. Fail-closed registries currently track all 599 top-level LibreOffice dialog roots (with explicit modal exclusions for input/destructive/credential/security prompts), 30 audited shipping text-query fields, a 1271-surface UI-closure ledger, dialog anatomy, eight routed notification producers, Impress/Draw surfaces, and the wave-2 shell/navigation contracts. Fifteen of the 30 search fields are source-integrated with the shared adjacent regex builder and 15 carry documented architectural gaps. The 2026-07-29 audit also wires notification selection/chain-ordered Undo/dismissal, document-tab ownership and single-subtraction layout, restart-based persisted accent resolution, Start Center/Templates search, AutoCorrect disabled-row semantics, and real Forms/Find/Quick-Find `i/g/m/s` engine routes. Start Center maintenance reloads preserve an active query, and boolean filter surfaces hide the unsupported Global option. The commit-auditable rewrite headline is 82.45% (1048/1271), with 223 pending. These are source/static/build-free facts; the current C++ follow-up has no local build or runtime proof. |
...
The separate 30-field coverage ledger records 15 source integrations and 15
documented gaps. Its parameterized source validator and 97 mutation regressions
...
dead-route bypasses. Calc Go to Sheet preserves its legacy
