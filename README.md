...
| Canonical rewrite coverage | Contracted; implementation in progress | The operator-provided design ZIP is pinned by hash in the [`Windows rewrite contract`](docs/design/00-windows-rewrite-contract.md). Its eleven surfaces govern the native Windows rewrite. Fail-closed registries currently track all 599 top-level LibreOffice dialog roots (with explicit modal exclusions for input/destructive/credential/security prompts), 30 audited shipping text-query fields, a 1271-surface UI-closure ledger, dialog anatomy, eight routed notification producers, Impress/Draw surfaces, and the wave-2 shell/navigation contracts. Fifteen of the 30 search fields are source-integrated with the shared adjacent regex builder and 15 carry documented architectural gaps. The 2026-07-31 slice normalizes the search dialog primary response and improves existing field-description panel spacing and accessibility metadata; XML, anatomy, a11y, and the 55-test ledger suite pass. The commit-auditable rewrite headline remains 82.61% (1050/1271), with 221 pending, because the fail-closed ledger does not credit the runtime-composed child panel automatically. These are source/static/build-free facts; native and runtime proof remain separate gates. |
