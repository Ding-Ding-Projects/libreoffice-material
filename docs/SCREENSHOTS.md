...
The whole [`screenshots/genuine/`](screenshots/genuine/) gallery was re-captured
on 2026-07-24 from the shipped
[`windows-msi-95-1-317f016605`](https://github.com/codingmachineedge/libreoffice-material/releases/tag/windows-msi-95-1-317f016605)
release binary (source `317f0166057e8d41a4f52470a2d5c1ceb26b5fc5`, the first
post-mega-wave build) and is registered in the
[2026-07-24 windows-msi-95 gallery](#2026-07-24--windows-msi-95-1-317f016605-genuine-suite-gallery)
section below: four Start Center harness captures (light, light keyboard focus,
dark, forced high contrast) with schema-v2 evidence-run manifests, plus
direct-MCP captures of all six applications (light and dark) and six shared
dialogs. Every image was read and visually confirmed; deviations from the
canonical `Libre Office.zip` design are recorded per entry in
[`screenshots/genuine/PROVENANCE.json`](screenshots/genuine/PROVENANCE.json)
as defects, per the operator directive that the zip design is the 100% target.
The previous gallery images from `windows-msi-89-1-705cf7ff4b` remain immutable
inside their 2026-07-23 evidence records; three msi-95 captures (Base wizard
light/dark, Print dialog) are byte-identical to the msi-89 ones, corroborating
deterministic rendering of unchanged surfaces.

**msi-95 regression note:** the rewritten Start Center renders its left-nav
pills without labels, lists no CREATE module entries, shows a Help-only footer,
and opens as a narrow portrait window; background pointer navigation to
Templates no longer lands, so the msi-95 gallery intentionally carries **no**
`start-center-templates-light.png` and the canonical count above stays at 9
(the 2026-07-20 `393263ad92` Templates evidence remains the last passing
Templates capture). Both defects are tracked as follow-up fixes against the
canonical zip design.
...
## 2026-07-24 — windows-msi-95-1-317f016605 genuine suite gallery

Source: the administratively extracted payload of release
[`windows-msi-95-1-317f016605`](https://github.com/codingmachineedge/libreoffice-material/releases/tag/windows-msi-95-1-317f016605)
(`program/soffice.exe`; MSI SHA-256
`751580e971ffd9bd2a9f6aaf23799a79c087f2bf9f7df73f081f2ab979aaf313`, payload
`version.ini` buildid equal to the source commit). Method: unedited per-window
`PrintWindow` on off-screen desktops via `lowlevel-computer-use-mcp`
`547a102a49169d41da876de217856229ab7c03a1`. Every entry's SHA-256 and
`visually_verified: true` flag is recorded in
[`screenshots/genuine/PROVENANCE.json`](screenshots/genuine/PROVENANCE.json)
(22 entries). Each image was read and visually confirmed to render genuine
Material; no image is a mockup or prototype render. Per-entry `visual_notes`
name every observed deviation from the canonical `Libre Office.zip` design.

### Start Center — smoke-harness runs (schema-v2 manifest + results + a11y tree)

| Appearance | Run directory | Screenshot(s) |
| --- | --- | --- |
| Light — Home + keyboard focus | [`20260724-133218-317f016605-windows-headless-light`](evidence/runs/20260724-133218-317f016605-windows-headless-light/) ([manifest](evidence/runs/20260724-133218-317f016605-windows-headless-light/manifest.json) / [results](evidence/runs/20260724-133218-317f016605-windows-headless-light/results.json)) | [`start-center-light`](screenshots/genuine/start-center-light.png) · [`start-center-light-keyboard-focus`](screenshots/genuine/start-center-light-keyboard-focus.png) |
| Light — Templates navigation attempt | [`20260724-133349-317f016605-windows-headless-light`](evidence/runs/20260724-133349-317f016605-windows-headless-light/) ([manifest](evidence/runs/20260724-133349-317f016605-windows-headless-light/manifest.json) / [results](evidence/runs/20260724-133349-317f016605-windows-headless-light/results.json)) | Retained as evidence only — the capture still shows Home because the Templates click no longer lands on the unlabeled nav; **not** promoted into the gallery |
| Dark — Home | [`20260724-133503-317f016605-windows-headless-dark`](evidence/runs/20260724-133503-317f016605-windows-headless-dark/) ([manifest](evidence/runs/20260724-133503-317f016605-windows-headless-dark/manifest.json) / [results](evidence/runs/20260724-133503-317f016605-windows-headless-dark/results.json)) | [`start-center-dark`](screenshots/genuine/start-center-dark.png) |
| Forced high contrast — Home | [`20260724-133552-317f016605-windows-headless-highcontrast`](evidence/runs/20260724-133552-317f016605-windows-headless-highcontrast/) ([manifest](evidence/runs/20260724-133552-317f016605-windows-headless-highcontrast/manifest.json) / [results](evidence/runs/20260724-133552-317f016605-windows-headless-highcontrast/results.json)) | [`start-center-highcontrast`](screenshots/genuine/start-center-highcontrast.png) |
...
passed the smoke harness's automation gate but retain
`pending_visual_review` scenario records in their committed candidates; the
application and dialog captures are genuine and visually confirmed but were
taken by direct MCP driving without the harness's scenario/manifest gate. All
are registered here and in `PROVENANCE.json` rather than promoted into the
canonical smoke gallery count. Formal `B V I A L P C` acceptance-gate credit
remains follow-up work, as does fixing the registered label/Create/footer and
Templates-navigation regressions against the canonical zip design.

## 2026-07-23 — windows-msi-89-1-705cf7ff4b genuine suite gallery (superseded)

> **Superseded 2026-07-24:** the `screenshots/genuine/` files this section's
> tables link to now contain the msi-95 re-captures above (except the three
> byte-identical ones). The msi-89 pixels remain retrievable from Git history
> at commit `791e65efc` and earlier, and the harness Start Center runs remain
> immutable under their `20260723-*` evidence-run directories.
...
| Light — Templates navigation | [`20260723-165153-705cf7ff4b-windows-headless-light-templates`](evidence/runs/20260723-165153-705cf7ff4b-windows-headless-light-templates/) ([manifest](evidence/runs/20260723-165153-705cf7ff4b-windows-headless-light-templates/manifest.json) / [results](evidence/runs/20260723-165153-705cf7ff4b-windows-headless-light-templates/results.json)) | [`start-center-templates-light`](evidence/runs/20260723-165153-705cf7ff4b-windows-headless-light-templates/screenshots/start-center-templates-light.png) (gallery copy removed 2026-07-24) |
