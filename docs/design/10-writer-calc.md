...
| Region | Size (prototype) | Tokens | Status |
| --- | --- | --- | --- |
| Document canvas | flexible width; vertical padding 26px; scrollable | `@surface-container-low` outside page-subtracted document rectangles | **source-composed, runtime unverified**: `SwViewShell::GetCanvasBackgroundColor()` keeps high contrast first, retains `GetAppBackgroundColor()` as the non-Material/invalid-palette fallback, and resolves the bundled light/dark `surface-container-low` role only under the exact Material gate. `PaintDesktop` subtracts visible page rectangles before filling the remaining workspace; the page-shadow helper clears all four bitmap seams with that same color. Document pixels, print/metafile output, LibreOfficeKit tiles, preview ownership, zoom, selection, and input coordinates stay on their existing paths. |
