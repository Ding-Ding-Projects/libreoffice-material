...
| `native:updater-lifecycle-ui` | **Source implemented, runtime unverified:** the shared Material notification card now carries exact version and stable lifecycle severity, state/progress summaries, paused/stalled percent, automatic-retry copy, and truthful Windows Installer completion/rollback ownership. The update icon remains the action entry point. No bundled verifier-backed code-name catalog/feed field exists, so the required graceful fallback shows version only and invents no code name. | Protected staged-MSI verification, default-No modal install consent, interactive launch, and both restart-suppression properties remain pinned by the updater lifecycle contract. |
